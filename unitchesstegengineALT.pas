...
FUNCTION figurenListeGenerieren():Tfigurenliste;
 ...
 (*
  new(neuefigur);
  neuefigur^.art:=cd;neuefigur^.farbe:=cschwarz;
  neuefigur^.pos:=C3;brett(.C3.):=cd*cschwarz;
  neuefigur^.geschlagen:=false;
  neuefigur^.nach:=nil;
  neuefigur^.vor:=aktuell;
  aktuell^.nach:=neuefigur;
  ...
  neuefigur^.pos:=D4;brett(.D4.):=cb*cschwarz;
  neuefigur^.geschlagen:=false;
  neuefigur^.nach:=nil;
  neuefigur^.vor:=aktuell;
  aktuell^.nach:=neuefigur;
...
VAR attacke,attackeverteidigt:BOOLEAN;
    figurbedroht,figurverteidigt,
    kleinsterwertangriff, kleinsterwertverteidigung:INTEGER;
...
BEGIN
  ...
  kleinsterwertangriff:=ck;
  kleinsterwertverteidigung:=ck;

  figurgef:=false;

  attacke:=false;
  attacke:=vonTurmBedroht(feld,auf);
  IF attacke THEN BEGIN figurbedroht := figurbedroht+1; IF ct < kleinsterwertangriff THEN kleinsterwertangriff:=ct END;
  ...
  IF attacke THEN BEGIN figurbedroht := figurbedroht+1; IF cl < kleinsterwertangriff THEN kleinsterwertangriff:=cl END;
  ...
  IF attacke THEN BEGIN figurbedroht := figurbedroht+1; IF cd < kleinsterwertangriff THEN kleinsterwertangriff:=cd END;
  ...
  IF attacke THEN BEGIN figurbedroht := figurbedroht+1; IF ck < kleinsterwertangriff THEN kleinsterwertangriff:=ck END;
  ...
  IF attacke THEN BEGIN figurbedroht := figurbedroht+1; IF cs < kleinsterwertangriff THEN kleinsterwertangriff:=cs END;
  ...
  IF attacke THEN BEGIN figurbedroht := figurbedroht+1; IF cb < kleinsterwertangriff THEN kleinsterwertangriff:=cb END;
  attacke:=vonBauerBedroht(feld,auf*aktuellefigur^.farbe+rechts);
  IF attacke THEN BEGIN figurbedroht := figurbedroht+1; IF cb < kleinsterwertangriff THEN kleinsterwertangriff:=cb END;

  (* Figur verteidigt BEGIN *)
  attackeVerteidigt:=false;
    attackeVerteidigt:=vonTurmGeschuetzt(feld,auf);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF ct < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=ct END;
    attackeVerteidigt:=vonTurmGeschuetzt(feld,links);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF ct < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=ct END;
    attackeVerteidigt:=vonTurmGeschuetzt(feld,rechts);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF ct < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=ct END;
    attackeVerteidigt:=vonTurmGeschuetzt(feld,ab);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF ct < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=ct END;

    attackeVerteidigt:=vonLaeuferGeschuetzt(feld,auf+links);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cl < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cl END;
    attackeVerteidigt:=vonLaeuferGeschuetzt(feld,ab+links);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cl < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cl END;
    attackeVerteidigt:=vonLaeuferGeschuetzt(feld,auf+rechts);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cl < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cl END;
    attackeVerteidigt:=vonLaeuferGeschuetzt(feld,ab+rechts);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cl < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cl END;


    attackeVerteidigt:=vonDameGeschuetzt(feld,auf);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cd < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cd END;
    attackeVerteidigt:=vonDameGeschuetzt(feld,links);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cd < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cd END;
    attackeVerteidigt:=vonDameGeschuetzt(feld,rechts);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cd < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cd END;
    attackeVerteidigt:=vonDameGeschuetzt(feld,ab);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cd < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cd END;


    attackeVerteidigt:=vonKoenigGeschuetzt(feld,auf+links);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF ck < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=ck END;
    attackeVerteidigt:=vonKoenigGeschuetzt(feld,auf+rechts);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF ck < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=ck END;
    attackeVerteidigt:=vonKoenigGeschuetzt(feld,ab+rechts);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF ck < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=ck END;
    attackeVerteidigt:=vonKoenigGeschuetzt(feld,ab+links);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF ck < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=ck END;
    attackeVerteidigt:=vonKoenigGeschuetzt(feld,auf);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF ck < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=ck END;
    attackeVerteidigt:=vonKoenigGeschuetzt(feld,ab);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF ck < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=ck END;
    attackeVerteidigt:=vonKoenigGeschuetzt(feld,rechts);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF ck < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=ck END;
    attackeVerteidigt:=vonKoenigGeschuetzt(feld,links);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF ck < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=ck END;


    attackeVerteidigt:=vonSpringerGeschuetzt(feld,auf+auf+links);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cs < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cs END;
    attackeVerteidigt:=vonSpringerGeschuetzt(feld,auf+auf+rechts);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cs < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cs END;
    attackeVerteidigt:=vonSpringerGeschuetzt(feld,ab+ab+rechts);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cs < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cs END;
    attackeVerteidigt:=vonSpringerGeschuetzt(feld,ab+ab+links);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cs < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cs END;
    attackeVerteidigt:=vonSpringerGeschuetzt(feld,links+links+auf);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cs < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cs END;
    attackeVerteidigt:=vonSpringerGeschuetzt(feld,links+links+ab);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cs < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cs END;
    attackeVerteidigt:=vonSpringerGeschuetzt(feld,rechts+rechts+auf);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cs < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cs END;
    attackeVerteidigt:=vonSpringerGeschuetzt(feld,rechts+rechts+ab);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cs < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cs END;



    attackeVerteidigt:=vonBauerGeschuetzt(feld,auf*aktuellefigur^.farbe+links);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cb < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cb END;
    attackeVerteidigt:=vonBauerGeschuetzt(feld,auf*aktuellefigur^.farbe+rechts);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cb < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cb END;
  ...
  IF (figurbedroht > c0) AND (figurverteidigt = c0) THEN figurgef:=true;


  IF (figurbedroht > c0) AND (figurverteidigt > c0)
     THEN
      BEGIN
...
VAR attacke,suchende:BOOLEAN;
 ...
 BEGIN
  attacke:= false;suchende:=false;
  testfeld:=feld+richtung;
  (*showmessage('Turmsuche'); *)
  WHILE ((brett(.testfeld.)<>dumy)AND(NOT(suchende)))
  DO
  BEGIN
   (*IF (brett(.testfeld.)<>(aktuellefigur^.farbe*aktuellefigur^.art))
   THEN *)
   (*showmessage(inttostr(aktuellefigur^.farbe)+'Testfeld='+inttostr(testfeld)+' figur='+inttostr(brett(.testfeld.))+' H1='+inttostr(brett(.H1.)));
    *)
      IF (
       (aktuellefigur^.farbe<c0)AND(brett(.testfeld.)=ct)
       OR
      ...
      THEN BEGIN (*showmessage('Turm gefunden');*)attacke:=true;suchende:=true;END;
      IF (brett(.testfeld.)<>leer) THEN suchende:=true;
   testfeld:=testfeld+richtung;
  END;
  vonTurmBedroht:=attacke;
...
BEGIN  (*showmessage(' -> H1='+inttostr(brett(.H1.)));*)
  figurbedroht:=c0;
  figurverteidigt:=c0;
  ...
  attacke:=false;
  attacke:=vonTurmBedroht(feld,auf);
  IF attacke THEN BEGIN (*showmessage('+1');*)figurbedroht := figurbedroht+1; IF ct < kleinsterwertangriff THEN kleinsterwertangriff:=ct END;
  attacke:=vonTurmBedroht(feld,links);
  IF attacke THEN BEGIN (*showmessage('+1');*)figurbedroht := figurbedroht+1; IF ct < kleinsterwertangriff THEN kleinsterwertangriff:=ct END;
  attacke:=vonTurmBedroht(feld,rechts);
  IF attacke THEN BEGIN (*showmessage('+1');*)figurbedroht := figurbedroht+1; IF ct < kleinsterwertangriff THEN kleinsterwertangriff:=ct END;
  ...
  attacke:=vonBauerBedroht(feld,auf*aktuellefigur^.farbe*-1+links);
  IF attacke THEN BEGIN figurbedroht := figurbedroht+1; IF cb < kleinsterwertangriff THEN kleinsterwertangriff:=cb END;
  attacke:=vonBauerBedroht(feld,auf*aktuellefigur^.farbe*-1+rechts);
  IF attacke THEN BEGIN figurbedroht := figurbedroht+1; IF cb < kleinsterwertangriff THEN kleinsterwertangriff:=cb END;
  ...
  attackeVerteidigt:=false;
    ...
    attackeVerteidigt:=vonBauerGeschuetzt(feld,auf*aktuellefigur^.farbe*-1+links);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cb < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cb END;
    attackeVerteidigt:=vonBauerGeschuetzt(feld,auf*aktuellefigur^.farbe*-1+rechts);
    IF attackeVerteidigt THEN BEGIN figurverteidigt := figurverteidigt+1; IF cb < kleinsterwertverteidigung THEN kleinsterwertverteidigung:=cb END;
...
END;
 ...
 BEGIN
  ...
  BEGIN
   IF ((aktuell^.art=cd)AND(aktuell^.farbe=farbe))
    THEN
    IF(figurgef(aktuell^.pos,aktuell)) THEN BEGIN damegef:=true; exit; END;
   aktuell:=aktuell^.nach;
  END;
 END;
 ...
 BEGIN
  ...
  BEGIN
   IF ((aktuell^.art=ct)AND(aktuell^.farbe=farbe))
    THEN
    IF(figurgef(aktuell^.pos,aktuell)) THEN BEGIN turmgef:=true; exit; END;
   aktuell:=aktuell^.nach;
  END;
 END;
 ...
 BEGIN (*alphabeta*)
    ...
    aktuell := zugliste;
       ...
       BEGIN
         IF aktuell^.geschlagen<>NIL THEN ruhigeStellung:=false;
         IF (aktuell <>nil) THEN aktuell := aktuell^.nach;
       END;
    zuglisteGegner:=nil;
    zuglisteGegner:= zuggenerator(-farbe,letzterzug);
    aktuellGegner := zuglisteGegner;
       WHILE aktuell <> NIL DO
       BEGIN
         IF aktuellGegner^.geschlagen<>NIL THEN ruhigeStellung:=false;
         IF (aktuellGegner <>nil) THEN aktuellGegner := aktuellGegner^.nach;
       END;
    IF (zuglisteGegner<>NIL)
        THEN zugAbbau(linie,-farbe,zuglisteGegner);
    ...
    BEGIN
       ...
       IF (tiefe < maxTiefe)OR((tiefe = maxTiefe) AND (aktuell^.geschlagen <>NIL)) THEN  (*ANFANG IF für ruhige Stellung*)
       BEGIN
	...
	BEGIN
        ...
        END; (* Ende IF für ruhige Stellung*)
   ...
   THEN
    ...
    DO
     ...
     IF ((aktuell^.art=ck)AND (aktuell^.pos=Ex)AND (aktuell^.farbe=farbe)AND (aktuell^.geschlagen=false))
      THEN
      BEGIN
       aktuell^.pos:=Cx;brett(.Cx.):=ck*farbe;brett(.Ex.):=leer;
      END;
     IF ((aktuell^.art=ct)AND (aktuell^.pos=Ax)AND (aktuell^.farbe=farbe)AND (aktuell^.geschlagen=false))
      THEN
      BEGIN
       aktuell^.pos:=Dx;brett(.Dx.):=ct*farbe;brett(.Ax.):=leer;
      END;
     aktuell:=aktuell^.nach;
     END;
    END;
...
VAR bewertung,zuglistenzaehler,schachzaehler: INTEGER;
     zugliste,aktuell:Tzugliste;
     zug,computerzieht:Tziehe;
     kannziehen,spielerZugNichtSchach:BOOLEAN;


BEGIN
    kannziehen:=false;spielerZugNichtSchach:=false;zuglistenzaehler:=c0;schachzaehler:=c0;
    IF (matt(linie,farbe,figurenliste))
    THEN
     BEGIN
      computerzugSPIELER:=false; writeln(' matt erkannt ');
      endmatt:=true;
     END
    ELSE
    BEGIN
       ...
       BEGIN
        ...
        (* ANFANG nicht schach *)
         ...
         IF ((Spielerzug.vonpos=aktuell^.vonpos)AND(Spielerzug.nachpos=aktuell^.nachpos)) THEN
         BEGIN
         spielerZugNichtSchach:=true;
      ...
      IF (kannziehen AND(zuglistenzaehler-schachzaehler>c0) AND spielerZugNichtSchach)
      THEN
      BEGIN
       write(linie,'COMPUTERZUG -> =');
       ...
       schreibePosition(computerzieht.nachpos);writeln;
       ComputerZugSetzen(brett,computerzieht);
       computerzugSPIELER:= true;
       (*Damenumwandlung---------------------------------------------------------------------------------------------------------------------------------------------------------------------*)
        IF ((computerzieht.art=cb)AND((brett(.computerzieht.nachpos+(10*computerzieht.farbe).)=dumy)))THEN bauernumwandlung(computerzieht.farbe,figurenliste,computerzieht);
       IF (zugliste<>nil)
       THEN BEGIN zugAbbau(linie,farbe,zugliste); zugliste:=NIL; END;
      END(*kannziehen*)
      ELSE BEGIN writeln ('kannziehen ',kannziehen,' zuglistenzaehler ',zuglistenzaehler,' schachzaehler ',schachzaehler); writeln('die Stellung ist patt'); patt:=true;computerzugSPIELER:= false;  END;

      END;(*ELSE MATT*)
  END;(*computerzugSPIELER*)
